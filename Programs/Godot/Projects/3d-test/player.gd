...
@onready var boxes_label = $HUD/BoxesLabel
...
func _ready():
	...
	Global.inventory_updated.connect(_on_invertory_updated)
	camera = camera1p
	raycast = raycast1p
	_on_invertory_updated()
...
func _physics_process(delta: float) -> void:
	...
	object = null
	boxes_label.text = "Boxes:   " + str(test_boxes).pad_zeros(3)
